...
import { Search, Filter, Eye, X, RefreshCw, Download } from 'lucide-react';
import * as Dialog from '@radix-ui/react-dialog';
import type { AuditLog } from '../types';

interface Props {
  logs?: AuditLog[]; // kept for backward compat but unused
}
...
export function AuditLogs({ logs: _ }: Props) {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/audit-log', { headers: { 'Authorization': `Bearer ${token}` } });
      const data = await res.json();
      if (Array.isArray(data)) {
        setLogs(data.map((l: any) => ({
          id: l.logID,
          userID: l.userID,
          tenNguoiDung: l.tenNguoiDung || 'Hệ thống',
          bangDuLieu: l.bangDuLieu || l.tenBang,
          hanhDong: l.hanhDong,
          thoiGian: l.thoiGian ? l.thoiGian.substring(0, 19).replace('T', ' ') : '',
          giaTriCu: l.giaTriCu || '',
          giaTriMoi: l.giaTriMoi || '',
          lyDoOverride: l.lyDoOverride || '',
        })));
      }
    } catch (err) {
  ...
  const exportCSV = () => {
    const headers = ['ID', 'Thời gian', 'Người dùng', 'Bảng dữ liệu', 'Hành động', 'Giá trị cũ', 'Giá trị mới', 'Lý do ghi đè'];
    const rows = filtered.map(l => [
      l.id, l.thoiGian, l.tenNguoiDung,
      TABLE_LABELS[l.bangDuLieu] || l.bangDuLieu,
      ACTION_LABELS[l.hanhDong] || l.hanhDong,
      l.giaTriCu, l.giaTriMoi, l.lyDoOverride || ''
  ...
  return (
    <div className="space-y-4">
      {/* Summary cards */}
      {loading ? (
        <div className="flex items-center justify-center py-10"><span className="inline-block w-6 h-6 border-2 border-teal-500 border-t-transparent rounded-full animate-spin" /></div>
      ) : (
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <button onClick={fetchLogs} disabled={loading} className="p-2 text-gray-400 hover:text-teal-600 hover:bg-teal-50 rounded-lg transition-colors" title="Làm mới">
          <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
        </button>
        <button onClick={exportCSV} disabled={filtered.length === 0} className="flex items-center gap-1.5 px-3 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed" title="Xuất CSV">
