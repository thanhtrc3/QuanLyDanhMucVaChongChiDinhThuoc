...
export function ContraindicationRules({ rules, setRules, medicines, currentUser, addAuditLog }: Props) {
  ...
  const selectMed = (val: string) => {
    const med = medicines.find(m => String(m.id) === val);
    if (med) {
      setForm(f => ({ ...f, thuocID: med.id, tenThuoc: med.tenThuong, maATC: med.maATC }));
    } else {
      setForm(f => ({ ...f, thuocID: 0, tenThuoc: '', maATC: '' }));
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editRule) {
        const res = await fetch(`/api/chong-chi-dinh/${editRule.id}`, {
          method: 'PUT',
        ...
        setRules([...rules, { ...form, id: data.data.id, tenThuoc: form.tenThuoc, maATC: form.maATC }]);
  ...
  const handleDelete = async (r: ContraindicationRule) => {
    try {
      const res = await fetch(`/api/chong-chi-dinh/${r.id}`, {
        method: 'DELETE',
