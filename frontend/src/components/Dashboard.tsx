...
export function Dashboard({ currentUser, medicines, patients, prescriptions }: DashboardProps) {
  const [stats, setStats] = useState({
    totalPatients: 0,
    totalPrescriptionsThisMonth: 0,
    nearExpiry: 0,
    monthlyWarnings: 0,
    chartData: Array.from({ length: 12 }, (_, i) => ({ month: `T${i + 1}`, donThuoc: 0 }))
  });
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStats({
          ...
          totalPrescriptionsThisMonth: data.totalPrescriptionsThisMonth,
          nearExpiry: data.nearExpiry,
          monthlyWarnings: data.monthlyWarnings,
          chartData: data.chartData.map((val: number, i: number) => ({ month: `T${i + 1}`, donThuoc: val }))
        });
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {canSeePatients && (
          <StatCard icon={Users} label="Tổng bệnh nhân" value={stats.totalPatients || patients.length} color="blue" delta="Từ hệ thống" />
        )}
        <StatCard icon={FileText} label="Đơn thuốc tháng này" value={stats.totalPrescriptionsThisMonth} color="teal" delta={`${pendingPrescriptions.length} chờ duyệt`} />
        ...
        <StatCard icon={ShieldAlert} label="Cảnh báo hệ thống" value={stats.monthlyWarnings} color="red" delta="Trong tháng này" />
      </div>
