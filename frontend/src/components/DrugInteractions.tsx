...
export function DrugInteractions({ interactions, setInteractions, medicines, currentUser, addAuditLog }: Props) {
  ...
  const selectMed1 = (val: string) => {
    const m = medicines.find(x => String(x.id) === val);
    if (m) setForm(f => ({ ...f, thuocID_1: m.id, tenThuoc_1: m.tenThuong, maATC_1: m.maATC }));
    else setForm(f => ({ ...f, thuocID_1: 0, tenThuoc_1: '', maATC_1: '' }));
  };
  const selectMed2 = (val: string) => {
    const m = medicines.find(x => String(x.id) === val);
    if (m) setForm(f => ({ ...f, thuocID_2: m.id, tenThuoc_2: m.tenThuong, maATC_2: m.maATC }));
    else setForm(f => ({ ...f, thuocID_2: 0, tenThuoc_2: '', maATC_2: '' }));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem) {
        const res = await fetch(`/api/tuong-tac-thuoc/${editItem.id}`, {
          method: 'PUT',
        ...
        const res = await fetch('/api/tuong-tac-thuoc', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` },
          body: JSON.stringify(form)
        });
        ...
        setInteractions([...interactions, { ...form, id: data.data.id }]);
  ...
  const executeDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/tuong-tac-thuoc/${deleteConfirm.id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      if (!res.ok) throw new Error('Xóa thất bại');
      
      setInteractions(interactions.filter(i => i.id !== deleteConfirm.id));
      addAuditLog('TuongTacThuoc', 'Xoa', `${deleteConfirm.tenThuoc_1} - ${deleteConfirm.tenThuoc_2}`, 'Đã xóa');
      toast.success('Đã xóa tương tác thuốc.');
  ...
  const handleCheck = () => {
    ...
    const found = interactions.find(i => (String(i.thuocID_1) === id1 && String(i.thuocID_2) === id2) || (String(i.thuocID_1) === id2 && String(i.thuocID_2) === id1));
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl w-full max-w-sm z-50 p-6">
            <div className="flex flex-col items-center text-center gap-3">
              ...
              <div className="flex gap-3 w-full mt-2">
                ...
                <button onClick={executeDelete} className="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium">Xóa</button>
              </div>
            </div>
          </Dialog.Content>
        </Dialog.Portal>
