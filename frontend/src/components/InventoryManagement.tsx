...
export function InventoryManagement({ medicines, setMedicines, currentUser, addAuditLog }: Props) {
  ...
  const handleAdjust = async () => {
    ...
    if (adjustType === 'sub' && adjustQty > oldQty) { toast.error('Số lượng xuất không được vượt quá tồn kho hiện tại.'); return; }
    const newQty = adjustType === 'add' ? oldQty + adjustQty : oldQty - adjustQty;
    
    try {
      const updatedMed = { ...adjustMed, tonKho: newQty };
      const res = await fetch(`/api/thuoc/${adjustMed.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!res.ok) throw new Error('Cập nhật tồn kho thất bại');

      setMedicines(medicines.map(m => m.id === adjustMed.id ? updatedMed : m));
  ...
  const getStockPct = (m: Medicine) => m.tonKhoToiThieu > 0 ? Math.min(100, (m.tonKho / (m.tonKhoToiThieu)) * 100) : 50;
