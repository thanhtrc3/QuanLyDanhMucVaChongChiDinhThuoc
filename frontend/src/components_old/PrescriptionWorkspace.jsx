...
export default function PrescriptionWorkspace({ medicines, onNavigate }) {
  ...
  const doseWarningToShow = (activeDoseWarning && activeDoseWarning.signature !== dismissedWarning) ? activeDoseWarning : null;
  ...
  async function saveDraft() {
    ...
    if (activeDoseWarning) {
      setDismissedWarning(''); // Force show warning
  ...
  return (
    <main className="app-shell">
      ...
      <DoseWarningDialog
        warning={doseWarningToShow}
        onOverride={overrideDoseWarning}
        onClose={() => {
          setDismissedWarning(doseWarningToShow?.signature || '');
        }}
      />
    </main>
  );
